...
class HProfileThread(QtCore.QThread):
    """
                ...
                for freq in self.freq_points:
                    try:
                        ...
                        for j in range(1, NUM_FRAMES + 1):
                            ...
                            # print(frame)
                            try:
                                image_buffer_copy = np.copy(self._hardware.camera.get_pending_frame_or_null().image_buffer)
                                self.update.emit(image_buffer_copy)
