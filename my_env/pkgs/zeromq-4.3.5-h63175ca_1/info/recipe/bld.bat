...
cmake -G "NMake Makefiles" -D ENABLE_DRAFTS=OFF -D WITH_PERF_TOOL=OFF -D ZMQ_BUILD_TESTS=ON -D ENABLE_CPACK=OFF -D CMAKE_BUILD_TYPE=Release -D CMAKE_INSTALL_PREFIX=%LIBRARY_PREFIX% -D ENABLE_CLANG=OFF -D ENABLE_LIBSODIUM_RANDOMBYTES_CLOSE=OFF -D ENABLE_CURVE=ON -D WITH_LIBSODIUM=ON ..
